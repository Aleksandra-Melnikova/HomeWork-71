...
const CartOneDish: React.FC<Props> = ({cartDish, onDelete}) => {
  const [showMenuItemsCart , setShowMenuItemsCart ] = useState<'d-block'|'d-none'>('d-block');
  useEffect(() => {
  ...
  return (
    <div className={`card mb-3 p-2 ${showMenuItemsCart}`}>
        <div className="row align-items-center justify-content-between">
          ...
          <button onClick={onDelete} className={'col-2 bg-transparent border-0 '} type={'button'}><Cart/></button>
        </div>
    </div>
  )
}
